/* =====================================================================
   Cahier de Physique-Chimie — serveur de la couche IA
   ---------------------------------------------------------------------
...
const app = express();
app.use(express.json({ limit: "16mb" })); // les photos et les PDF arrivent en base64
...
function cleQuota() {
  return "cahier-physique:quota:" + new Date().toISOString().slice(0, 10);
}
function enteteKV() {
...
const SYSTEME_TUTEUR = `Tu es professeur de physique-chimie en classe de Première (spécialité physique-chimie, programme français). Tu aides une élève débutante qui a besoin d'explications simples.
...
- Vérifie chacun de tes calculs avant de l'écrire, unités comprises. Une correction fausse est pire que pas de correction.`;
...
/* Un PDF part tel quel vers l'API : Claude le lit lui-même, y compris
   quand il s'agit de pages scannées, que l'extraction locale ne sait pas
   décoder. Limite raisonnable : quelques mégaoctets. */
function blocPdf(dataUrl) {
  const m = /^data:application\/pdf;base64,(.+)$/i.exec(String(dataUrl || ""));
  if (!m) throw new Error("le fichier reçu n'est pas un PDF");
  const octets = Math.floor((m[1].length * 3) / 4);
  if (octets > 9 * 1024 * 1024)
    throw new Error("PDF trop lourd (9 Mo maximum) — envoie seulement les pages du cours");
  return {
    type: "document",
...
async function testerEcriture() {
  ...
  if (KV_URL && KV_TOKEN) {
    const cle = "cahier-physique:diag";
...
const KV_CLE = "cahier-physique:etat";
...
const ExoNum = z.object({
  ...
  rep: z.number(),
  tol: z.number(),
  // en physique-chimie, un résultat sans unité ne veut rien dire
  unite: z.string(),
...
const SYSTEME_FICHE = `Tu es professeur de physique-chimie en Première (spécialité physique-chimie, programme français). Tu transformes un cours en fiche pédagogique pour une élève débutante.
...
app.post("/api/fiche", autorise, async (req, res) => {
  if (tropRapide(req))
    return res.status(429).json({ erreur: "trop de demandes d'affilée — attends une minute" });
  const { titre, texte, image, pdf } = req.body || {};
  if (!texte && !image && !pdf)
  ...
  try {
    ...
    if (pdf) content.push(blocPdf(pdf));
    content.push({
      type: "text",
      text:
        (titre ? `Titre indiqué par l'élève : ${titre}\n\n` : "") +
        (image ? "Le cours est sur l'image ci-dessus.\n\n" : "") +
        (pdf ? "Le cours est dans le PDF ci-dessus. Lis-le en entier, y compris les schémas et les tableaux.\n\n" : "") +
...
function verifier(fiche) {
  ...
  const exercices = (fiche.exercices || []).filter((x, i) => {
    if (x.type === "qcm") {
      ...
      x.tol = tol;
      x.unite = typeof x.unite === "string" ? x.unite.trim() : "";
...
app.listen(PORT, () => {
  console.log(`\n  Cahier de Physique-Chimie — http://localhost:${PORT}`);
